...
def train_one_epoch(__C,
    ...
    for ith_batch, data in loader:
        ...
        if ni < __C.BURN_UP:
            ...
            teacher.eval()
            with torch.no_grad():
                pseudo_box, pseudo_mask = teacher(image_iter_unlabel_w, ref_iter_unlabel_w)
            ...
            if scalar is not None:
                with th.cuda.amp.autocast():
                    loss_sup,loss_det_sup,loss_seg_sup,loss_semi,loss_det_semi,loss_seg_semi = student(image_iter,ref_iter,det_label=box_iter,seg_label=mask_iter,semi=True,weight=weight)
            else:
                loss_sup,loss_det_sup,loss_seg_sup,loss_semi,loss_det_semi,loss_seg_semi = student(image_iter,ref_iter,det_label=box_iter,seg_label=mask_iter,semi=True,weight=weight)
...
def main():
    ...
    # parser.add_argument('--config', type=str, default='./config/SimREC_RefCOCO_scratch_realgin_semi_augment_weight.yaml')
