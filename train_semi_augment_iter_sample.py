...
from test import validate,validate_semi
from utils.utils import  EMA
from collections import OrderedDict
import torch
import torchvision.ops as ops

def random_sample(number):
    for i in range(number.shape[0]):
        sample = torch.randint(0,int(number[i]),(1,1))
        if i == 0:
            index = sample.clone()
        else:
            index = torch.cat([index, sample],0)
    return index
...
def validate_iter(__C,
    ...
    with th.no_grad():
        ...
        for ith_batch, data in enumerate(loader):
            ...
            box, mask, pseudo_box= net(image_iter, ref_iter)
...
def train_one_epoch(__C,
    ...
    for ith_batch, data in loader:
        ...
        if ni < __C.BURN_UP:
            ...
            teacher.eval()
            with torch.no_grad():
                pseudo_box, pseudo_mask, pseudo_sample_box = teacher(image_iter_unlabel_w, ref_iter_unlabel_w)
                pseudo_sample_box_nms = torch.zeros(pseudo_sample_box.size(0),3,pseudo_sample_box.size(2)).cpu().numpy()
                ...
                # pseudo_sample_box=pseudo_sample_box.squeeze(1).cpu().numpy()

                score=pseudo_sample_box[:,:,4] # [B,507]
                ind = score.argsort()[:,-3:] # [B,3]
                ind_index = ind.unsqueeze(2).repeat(1,1,5)
                pseudo_sample_box=torch.gather(pseudo_sample_box,1,ind_index) # [B,1,5]
                pseudo_sample_box=pseudo_sample_box.squeeze(1).cpu().numpy()

                # top3 nms sample
                ###predictions to gt
                for i in range(len(gt_box_iter_unlabel_w)):
                    for j in range(pseudo_sample_box.shape[1]):
                        pseudo_sample_box[i][j]=yolobox2label(pseudo_sample_box[i][j],info_iter_unlabel_w[i])
                
                
                number = np.zeros([len(gt_box_iter_unlabel_w)], dtype=np.int32)
                for i in range(len(gt_box_iter_unlabel_w)):
                    output = ops.nms(boxes=torch.from_numpy(pseudo_sample_box[i,:, :4]), scores=torch.from_numpy(pseudo_sample_box[i,:,4]), iou_threshold=0.45)
                    number[i] = output.size(0)
                    pseudo_sample_box_nms[i,:output.size(0),:5] = pseudo_sample_box[i,output,:5]
                pseudo_sample_box_nms = torch.from_numpy(pseudo_sample_box_nms).clone()
                
                index = random_sample(number) # random select 1 [B,1]
                index = index.unsqueeze(1).repeat(1,1,5)
                pseudo_sample_box=torch.gather(pseudo_sample_box_nms,1,index) # [B,1,5]
                pseudo_sample_box=pseudo_sample_box.squeeze(1).cpu().numpy()

                pseudo_sample_box[:, 2] = (pseudo_sample_box[:, 2]-pseudo_sample_box[:, 0])
                pseudo_sample_box[:, 3] = (pseudo_sample_box[:, 3]-pseudo_sample_box[:, 1])
                from datasets.dataloader import label2yolobox
                sized_pseudo_sample_box = np.zeros((len(gt_box_iter_unlabel_w),5))
                for i in range(len(gt_box_iter_unlabel_w)):
                    sized_pseudo_sample_box[i]=label2yolobox(pseudo_sample_box[i].reshape(1,-1),tuple(info_iter_unlabel_q[i]),__C.INPUT_SHAPE[0],lrflip=__C.FLIP_LR)
                sized_pseudo_sample_box = torch.from_numpy(sized_pseudo_sample_box[:, :4]).cuda(non_blocking=True)
                pseudo_mask = pseudo_mask.unsqueeze(1)
                sized_pseudo_sample_box = sized_pseudo_sample_box.unsqueeze(1)

            if __C.FILTER == True:
                filter = pseudo_sample_box[:,4] > __C.FILTER_THRE
                if False in filter:
                    ...
                    sized_pseudo_sample_box = sized_pseudo_sample_box[chosen_index[0],...].clone()
                    pseudo_mask = pseudo_mask[chosen_index[0],...].clone() 

            check_x = sized_pseudo_sample_box[...,0]>=1
            check_y = sized_pseudo_sample_box[...,1]>=1
            ...
            box_iter=torch.cat([box_iter,sized_pseudo_sample_box],0).cuda(non_blocking=True)
        ...
        if step % __C.VALIDATE_FREQ == 0 and step != 0:
            if __C.USE_EMA == True:
                ...
                box_ap_student_noema,mask_ap=validate_iter(__C,student,val_loader,writer,epoch,step,rank,ix_to_token,ema=None,student=True)
            else:
                box_ap_student,mask_ap=validate_iter(__C,student,val_loader,writer,epoch,step,rank,ix_to_token,ema=ema,student=True)
...
def main():
    ...
    # parser.add_argument('--config', type=str, default='./config/SimREC_RefCOCO_scratch_realgin_semi_augment_sample.yaml')
