...
def train_one_epoch(__C,
    ...
    for ith_batch, data in loader:
        ...
        # mask_iter_unlabel = mask_iter_unlabel.cuda(non_blocking=True)
        # box_iter_unlabel = box_iter_unlabel.cuda(non_blocking=True)
...
def main_worker(gpu,__C):
    ...
    # scheduler = get_lr_scheduler(__C,optimizer,min(len(train_loader_label),len(train_loader_unlabel)))
    # scheduler_sup = get_lr_scheduler(__C,optimizer,min(len(train_loader_label),len(train_loader_unlabel)))
    ...
    for ith_epoch in range(start_epoch, __C.EPOCHS):
        ...
        # if __C.RANK != -1:
        #     train_loader_label.sampler.set_epoch(ith_epoch)
        #     train_loader_unlabel.sampler.set_epoch(ith_epoch)
        ...
        # train_loader = enumerate(zip(train_loader_label,train_loader_unlabel))
        ...
        if ith_epoch>=__C.BURN_UP_EPOCH:
