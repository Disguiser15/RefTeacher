...
class head(nn.Module):
    ...
    def __init__(self, __C, layer_no, in_ch, ignore_thre=0.5):
        ...
        self.bs_label = __C.BATCH_SIZE//len(__C.GPU)
    ...
    def forward(self, xin,yin, x_label=None,y_label=None,semi=False,confidence_map=None):
        """
        In this
        Args:
        ...
        """
        output = self.dconv(xin) # xin:[8,1024,13,13] output:[8,15,13,13]

        # mask=self.sconv(yin)
        # print(output.size(),mask.size())

        batchsize = output.shape[0]
        fsize = output.shape[2] # 13
        n_ch = 5 + self.n_classes
        dtype = torch.cuda.FloatTensor if xin.is_cuda else torch.FloatTensor
        devices=xin.device

        output = output.view(batchsize, self.n_anchors, n_ch, fsize, fsize) # [8,3,5,13,13]
        output = output.permute(0, 1, 3, 4, 2).contiguous() # [8,3,13,13,5]

        # sharpen
        output_pre = output[...,4].clone().view(batchsize, -1)
        p = torch.softmax(output_pre, dim=1)
        pt = p**(1/0.5)
        targets_u = pt / pt.sum(dim=1, keepdim=True)
        targets_u = targets_u.view(batchsize,self.n_anchors,fsize,fsize)
        ...
        x_shift = dtype(np.broadcast_to(
            np.arange(fsize, dtype=np.float32), output.shape[:4])).to(devices) # [8,3,13,13]
        y_shift = dtype(np.broadcast_to(
            np.arange(fsize, dtype=np.float32).reshape(fsize, 1), output.shape[:4])).to(devices) # [8,3,13,13]

        masked_anchors = np.array(self.masked_anchors) # (3,2)

        w_anchors = dtype(np.broadcast_to(np.reshape(
            masked_anchors[:, 0], (1, self.n_anchors, 1, 1)), output.shape[:4])).to(devices) # [8,3,13,13]
        h_anchors = dtype(np.broadcast_to(np.reshape(
            masked_anchors[:, 1], (1, self.n_anchors, 1, 1)), output.shape[:4])).to(devices) # [8,3,13,13]

        pred = output.clone() # [8,3,13,13,5]
        pred[..., 0] += x_shift
        pred[..., 1] += y_shift
        ...
        if x_label is None:  # not training
            ...
            pred[..., :4] *= self.stride
            pred=pred.view(batchsize,-1,n_ch) # [8,507,5]
            #xc,yc,w,h->xmin,ymin,xmax,ymax
            pred[:, :, 0] = pred[:, :, 0] - pred[:, :, 2] / 2
            pred[:, :, 1] = pred[:, :, 1] - pred[:, :, 3] / 2
            pred[:, :, 2] = pred[:, :, 0] + pred[:, :, 2]
            pred[:, :, 3] = pred[:, :, 1] + pred[:, :, 3]
            score=pred[:,:,4].sigmoid() # [8,507]
            ind=torch.argmax(score,-1).unsqueeze(1).unsqueeze(1).repeat(1,1,n_ch) # [8]->[8,1,1]->[8,1,5] torch.argmax(..., dim=1) row find max
            pred=torch.gather(pred,1,ind) # [8,1,5]
            return pred.view(batchsize,-1),torch.zeros(batchsize,fsize*self.stride,fsize*self.stride).to(pred.device), confidence_map.to(pred.device) # torch.mul(confidence_map, targets_u).to(pred.device)
        ...
        tgt_mask = torch.zeros(batchsize, self.n_anchors,
                               fsize, fsize, 4 + self.n_classes).type(dtype).to(devices) # [8,3,13,13,4]
        obj_mask = torch.ones(batchsize, self.n_anchors,
                              fsize, fsize).type(dtype).to(devices) # [8,3,13,13]
        tgt_scale = torch.zeros(batchsize, self.n_anchors,
                                fsize, fsize, 2).type(dtype).to(devices) # [8,3,13,13,2]

        target = torch.zeros(batchsize, self.n_anchors,
                             fsize, fsize, n_ch).type(dtype).to(devices) # [8,3,13,13,5]

        x_label = x_label.cpu().data # [8,1,4]
        nlabel = (x_label.sum(dim=2) > 0).sum(dim=1)  # number of objects [8]

        truth_x_all = x_label[:, :, 0] * fsize # [8,1]
        ...
        # output: [8,3,13,13,5] obj_mask: [8,3,13,13] tgt_mask: [8,3,13,13,4] tgt_scale: [8,3,13,13,2]
        ...
        bceloss_sup = nn.BCELoss(weight=tgt_scale[:self.bs_label,...]*tgt_scale[:self.bs_label,...])  # weighted BCEloss
        if semi == False:
            ...
            loss_xy_sup = bceloss_sup(output[:self.bs_label,..., :2], target[:self.bs_label,..., :2])
            ...
            if output.size(0) != self.bs_label:
                ...
                loss_obj_unsup = self.bce_loss(output[self.bs_label:,..., 4], target[self.bs_label:,..., 4]) + 0.1 * self.bce_loss(output[self.bs_label:,..., 4], confidence_map)
            else:
            ...
            loss_det_sup*=(float(self.bs_label))
            loss_det_unsup*=(float(batchsize-self.bs_label))
