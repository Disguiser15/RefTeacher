...
from models.simrec_semi_refteacher.head import REChead
...
class Net(nn.Module):
    ...
    def forward(self,x,y, det_label=None,seg_label=None,semi=False,att_target=None,confidence_map=None,weight=None):
        ...
        top_feats,m_attn,attn=self.attention_manner(y['flat_lang_feat'],x[-1])
        bot_feats=x[0]
        if self.training:
            if semi==True:
                loss,loss_det,loss_seg,loss_semi,loss_det_semi,loss_seg_semi=self.head(top_feats,bot_feats,det_label,seg_label,semi=True,att_prediction=attn,att_target=att_target,confidence_map=confidence_map,weight=weight)
                return loss,loss_det,loss_seg,loss_semi,loss_det_semi,loss_seg_semi
            else:
                loss,loss_det,loss_seg=self.head(top_feats,bot_feats,det_label,seg_label,semi==False)
                return loss,loss_det,loss_seg
        else:
            box, mask, confidence_map=self.head(top_feats,bot_feats)
