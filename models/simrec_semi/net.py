...
from models.simrec_semi.head import REChead
...
class Net(nn.Module):
    ...
    def forward(self,x,y, det_label=None,seg_label=None,semi=False):
        ...
        if self.training:
            # if semi==True:
            #     loss,loss_det,loss_seg=self.head(top_feats,bot_feats,det_label[ :det_label.size(0)//2,:,:],seg_label)
            #     loss_semi,loss_det_semi,loss_seg_semi=self.head(top_feats,bot_feats,det_label[det_label.size(0)//2: ,:,:],seg_label)
            #     return loss,loss_det,loss_seg,loss_semi,loss_det_semi,loss_seg_semi
            if semi==True:
                loss,loss_det,loss_seg,loss_semi,loss_det_semi,loss_seg_semi=self.head(top_feats,bot_feats,det_label,seg_label,semi=True)
                return loss,loss_det,loss_seg,loss_semi,loss_det_semi,loss_seg_semi
            else:
