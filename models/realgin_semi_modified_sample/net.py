...
from models.realgin_semi_modified_sample.head import head
...
class Net(nn.Module):
    ...
    def forward(self, x,y, det_label=None,seg_label=None,semi=False):
        ...
        if self.training:
            if semi==True:
                loss,loss_det,loss_seg,loss_semi,loss_det_semi,loss_seg_semi=self.head(x,None,det_label,seg_label,semi=True) # x: (B,1024,8,8)
            ...
            # if semi==True:
            #     loss,loss_det,loss_seg=self.head(x,None,det_label,seg_label,semi=True)
            # else:
            #     loss,loss_det,loss_seg=self.head(x,None,det_label,seg_label,semi=False)
            # return loss,loss_det,loss_seg
        else:
            box, mask, pseudo_box=self.head(x,None)
            return box, mask, pseudo_box
