...
from models.realgin_semi_modified_weight.head import head
...
class Net(nn.Module):
    def __init__(self, __C, pretrained_emb, token_size):
        ...
        self.head=head(__C,0,__C.HIDDEN_SIZE)
    ...
    def frozen(self,module):
        if getattr(module,'module',False):
            ...
            for param in module.parameters():
                param.requires_grad = False
    def forward(self, x,y, det_label=None,seg_label=None,semi=False,weight=None):
        x=self.visual_encoder(x) # [B,256-512-1024,32-16-8,32-16-8]
        ...
        if self.training:
            if semi==True:
                loss,loss_det,loss_seg,loss_semi,loss_det_semi,loss_seg_semi=self.head(x,None,det_label,seg_label,semi=True,weight=weight) # x: (B,1024,8,8)
                return loss,loss_det,loss_seg,loss_semi,loss_det_semi,loss_seg_semi
            else:
                loss,loss_det,loss_seg=self.head(x,None,det_label,seg_label,semi=False,weight=None)
